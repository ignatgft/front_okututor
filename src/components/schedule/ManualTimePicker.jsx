...
export default function ManualTimePicker({
  ...
  const [end, setEnd] = useState(initialEnd || addMinutes(initialStart, duration));
  const [checking, setChecking] = useState(false);
  const [status, setStatus] = useState(null); // {available, reason, message, suggestedSlots}
  ...
  // keep end in sync when start changes (auto +duration) if not manually edited recently
  ...
  useEffect(() => {
    ...
    if (!tutorId) {
      // no tutor to check — just validate format
      ...
      try {
        ...
        if (response.ok) {
          const normalized = {
            ...
            suggestedSlots: data.suggestedSlots || data.suggested_slots || [],
          };
          setStatus(normalized);
          onValidityChange?.(!!normalized.available);
        } else {
          setStatus({ available: false, reason: data?.reason || "INVALID", message: data?.message || t("schedule.manual.invalid", "Некорректное время"), suggestedSlots: data?.suggestedSlots || data?.suggested_slots || [] });
          onValidityChange?.(false);
        }
      } catch {
        setStatus({ available: false, reason: "ERROR", message: t("schedule.manual.check_failed", "Не удалось проверить"), suggestedSlots: [] });
        onValidityChange?.(false);
      } finally {
        setChecking(false);
      }
  ...
  }, [start, end, date, tutorId, timezone, touched, t, onValidityChange]);
      ...
      <div className="manual-time__inputs">
        <label className="manual-time__field">
          ...
          <input
            ...
            maxLength={5}
            aria-label={t("schedule.manual.start", "Начало")}
          />
        </label>
        ...
        <label className="manual-time__field">
          ...
          <input
            ...
            maxLength={5}
            aria-label={t("schedule.manual.end", "Конец")}
          />
        </label>
      </div>
