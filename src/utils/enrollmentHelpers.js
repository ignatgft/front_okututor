...
export function isTerminalStatus(status) {
  return [ENROLLMENT_STATUS.REJECTED, ENROLLMENT_STATUS.CANCELLED, ENROLLMENT_STATUS.COMPLETED, ENROLLMENT_STATUS.EXPIRED].includes(status);
}

export function canStudentMessage(status) {
  return !isTerminalStatus(status) && !!status;
}

export function canTutorMessage(status) {
  return !!status && status !== ENROLLMENT_STATUS.EXPIRED;
}

export function canAssignSchedule(status) {
  return [ENROLLMENT_STATUS.PENDING, ENROLLMENT_STATUS.ACCEPTED].includes(status);
}

export function canReschedule(status) {
  return [ENROLLMENT_STATUS.SCHEDULE_PENDING, ENROLLMENT_STATUS.SCHEDULE_PROPOSED].includes(status);
}

export function canViewScheduleProposal(status) {
  return [ENROLLMENT_STATUS.SCHEDULE_PROPOSED, ENROLLMENT_STATUS.SCHEDULE_PENDING, ENROLLMENT_STATUS.SCHEDULED].includes(status);
}

export function canStudentCancel(status) {
  return [ENROLLMENT_STATUS.PENDING, ENROLLMENT_STATUS.NEEDS_INFO, ENROLLMENT_STATUS.ACCEPTED, ENROLLMENT_STATUS.SCHEDULE_PROPOSED, ENROLLMENT_STATUS.SCHEDULE_PENDING].includes(status);
}

export function canStudentViewSchedule(status) {
  return status === ENROLLMENT_STATUS.SCHEDULED;
}

export function canStudentConfirmSchedule(status) {
  // backend may not have confirm; we expose for future
  return status === ENROLLMENT_STATUS.SCHEDULE_PROPOSED;
}

export function canTutorAct(status) {
  return [ENROLLMENT_STATUS.PENDING, ENROLLMENT_STATUS.NEEDS_INFO, ENROLLMENT_STATUS.ACCEPTED].includes(status);
}

export function canTutorRequestInfo(status) {
  return status === ENROLLMENT_STATUS.PENDING || status === ENROLLMENT_STATUS.ACCEPTED;
}

export function canTutorReject(status) {
  return canTutorAct(status);
}

export function canJoinLesson(startAt, now = Date.now()) {
...
export function getNextStudentAction(status) {
  switch (status) {
...
export function getNextTutorAction(status) {
  switch (status) {
    case ENROLLMENT_STATUS.PENDING:
...
export function openDirectChat(navigate, role, peerUserId) {
